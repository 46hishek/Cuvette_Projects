...
const Modal = ({ isOpen, onClose, setGroupNames, setColor }) => {
  ...
  const handleInput = () => {
    if (input.trim().length >= 20) {
      setColorError("Group Name should be less than 20 Characters");
  ...
  const handleGroupName = () => {
    if (input.trim() !== "" && selectedColor.trim() !== "") {
      ...
      if (input.trim() === "" && selectedColor.trim() === "") {
        setColorError("Provide Group Name and Color, Please!!!");
      } else if (input.trim() === "") {
        setColorError("Enter the Group Name, Please!!!");
      } else {
        setColorError("Select a Color, Please!!!");
      }
    }
  };
